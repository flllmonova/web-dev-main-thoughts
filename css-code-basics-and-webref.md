...
3. Дочерний селектор (descendant selector)
```
p strong,
#fancy span {
  color: red;
...
4. Селектор класса (class selector)
- комбинирование с селектором типа
```
.red,
p.red {  
...
5. Несколько селекторов (comma combinator)
```
h1, p {
  color: red;
}
...
7. Селектор брасткого элемента после определенного братского `A + B` (adjacent sibling selector)
